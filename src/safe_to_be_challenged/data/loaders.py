...
from ..config import ABSTRACTS_PATH, CONDITIONS, FAST_MODE, MACSS_PATH
...
class CorpusLoader:
    ...
    def load(self) -> list[dict]:
        """Load corpus from JSON. Returns sample data if file missing/empty."""
...
class DoublesLoader:
    ...
    def load_doubles_from_corpus(
        ...
        """Build doubles: (thesis_abstract, condition, trust_sensitivity, skill_profile)."""
        ...
        for cond in CONDITIONS:
            for _ in range(n_per_condition):
                ...
                doubles.append({
                    "thesis": thesis,
                    "condition": cond,
                    "trust_sensitivity": "low" if (idx + hash(cond) % 2) == 0 else "high",
