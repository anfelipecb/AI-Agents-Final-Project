...
def parse_oai_record(rec: ET.Element) -> dict | None:
    ...
    # Full abstract as provided by the repository (OAI dc:description; we do not truncate)
...
def harvest_theses(max_records: int | None = 50, *, oai_url: str = OAI_URL) -> list[dict]:
    """Harvest thesis records via OAI-PMH ListRecords. Use max_records=0 or None to fetch all theses."""
    LOGS_DIR.mkdir(exist_ok=True)
    theses: list[dict] = []
    token: str | None = None
    initial_params = {"verb": "ListRecords", "metadataPrefix": "oai_dc", "set": "Theses"}
    no_limit = max_records is None or max_records <= 0

    with httpx.Client(timeout=60.0) as client:
        while no_limit or len(theses) < max_records:
            ...
            for rec in root.findall(".//oai:record", NS):
                ...
                if not no_limit and len(theses) >= max_records:
                    break
...
def run_harvest(max_records: int | None = 50) -> Path:
    """Harvest theses and save to MACSS_PATH. Use max_records=0 to fetch all theses."""
