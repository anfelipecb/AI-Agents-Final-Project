"""Core metrics: mechanical_reliance, trust, quality, extract_rating."""
import re
...
def feedback_specificity(feedback_text: str, generate_fn) -> int:
    """LLM rates feedback specificity (1=generic, 5=highly targeted and actionable)."""
    prompt = f"Rate feedback specificity (1=generic/vague, 5=highly targeted and actionable). Feedback: {feedback_text[:400]}"
    out = generate_fn(prompt, max_new_tokens=5, temperature=0.1, do_sample=False)
    return extract_rating(out)
